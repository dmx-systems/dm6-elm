...
viewTopic topic props mapId model =
    let
        ( style, children ) =
            topicFunc topic props mapId model
...
relPos pos mapId model =
    absMapPos mapId (Point 0 0) model
        |> Maybe.andThen
            (\posAbs ->
                Just <|
                    Point
        ...
        |> Maybe.andThen
            (\map ->
                ...
                if isFullscreen mapId model then
                    ...
                    getTopicPos map.id map.parentMapId model.maps
                        |> Maybe.andThen
                            (\mapPos ->
                                absMapPos
                                    map.parentMapId
    ...
    let
        targeted =
            case model.mouse.dragState of
                ...
                Drag DrawAssoc _ mapId_ _ _ (Just target) ->
